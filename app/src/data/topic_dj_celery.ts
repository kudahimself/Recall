...
False`,
    ...
    // 1. Coding: Define a Celery task with @shared_task
  {
      ...
      tieredHints: {
        apiSignature: 'send_mail(subject, message, from_email, recipient_list) -> int',
        skeleton: '# myapp/tasks.py\nfrom celery import ____\nfrom django.core.mail import ____\n\n\n@____\ndef send_welcome_email(user_email, username):\n    """Send a welcome email to a new user."""\n    ____(\n        subject="Welcome!",\n        message=f"Hello {username}, welcome to our platform!",\n        from_email="noreply@example.com",\n        recipient_list=[user_email],\n    )\n    return f"Email sent to {user_email}"\n\n\n# Calling the task asynchronously (e.g., in a view):\n# send_welcome_email.____("user@example.com", "Alice")',
      },
      hints: [
...
app.___()`,
    ...
    // 2. Coding: Configure Celery in a Django project
  {
      ...
      tieredHints: {
        apiSignature: 'app.config_from_object(obj, silent=False, force=False, namespace=None)',
        skeleton: '# myproject/celery.py\nimport os\nfrom celery import Celery\n\n# Set default Django settings module\nos.environ.setdefault("DJANGO_SETTINGS_MODULE", "myproject.settings")\n\napp = Celery("myproject")\n\n# Read config from Django settings, namespace=\'CELERY\' means\n# all celery settings must be prefixed with CELERY_ in settings.py\napp.____("django.conf:settings", namespace="CELERY")\n\n# Auto-discover tasks.py in all installed apps\napp.____()\n\n\n# myproject/__init__.py\nfrom .celery import app as ____\n\n__all__ = ("____",)',
      },
      hints: [
    ...
    // 5. Coding: Task with retry on failure
  {
      ...
      tieredHints: {
        apiSignature: 'self.retry(args=None, kwargs=None, exc=None, throw=True, eta=None, countdown=None, max_retries=None)',
        skeleton: '# myapp/tasks.py\nfrom celery import shared_task\nimport requests\n\n\n@shared_task(____=True, max_retries=3)\ndef fetch_user_data(self, user_id):\n    """Fetch user data from external API with retry on failure."""\n    try:\n        response = requests.get(\n            f"https://api.example.com/users/{user_id}",\n            timeout=10,\n        )\n        response.____()\n        return response.json()\n    except requests.RequestException as exc:\n        # Retry in 60 seconds, up to 3 times\n        raise self.____(exc=exc, countdown=60)\n\n\n# Usage:\n# fetch_user_data.delay(42)',
      },
      hints: [
...
parallel.apply_async()`,
    ...
    // 8. Coding: Chain and group
  {
      ...
      tieredHints: {
        apiSignature: 'chain(*tasks) | group(*tasks) -> Signature',
        skeleton: 'from celery import shared_task, chain, group\n\n# Chain: fetch_data result flows into process_data\n# .s() creates a "signature" (serializable task reference)\nworkflow = ____(\n    ____.____("https://api.example.com/data"),\n    ____.____(),\n)\nresult = workflow.____()\n\n# Group: run 3 notifications in parallel\nnotifications = ____(\n    ____.____(1),\n    ____.____(2),\n    ____.____(3),\n)\ngroup_result = notifications.____()',
      },
      hints: [
...
def process_image(path):
    ...
    return workflow.apply_async()`,
      ...
      tieredHints: {
        apiSignature: 'chord(header, body=None, task_id=None) -> Signature',
        skeleton: 'from celery import chain, group, chord\nfrom .tasks import resize_image, extract_metadata, make_thumbnail, save_to_db\n\n\ndef process_image(path):\n    workflow = ____(\n        ____.____(path),\n        ____(\n            ____(____.____(), ____.____()),\n            ____.____(),\n        ),\n    )\n    return workflow.____()',
      },
      hints: [
