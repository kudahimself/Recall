...
products_by_brand = Product.objects.annotate(
    brand=KeyTextTransform("brand", "metadata")
      ...
      tieredHints: {
        apiSignature: 'models.JSONField(default=dict, blank=True); KeyTextTransform(key_name, field_name)',
        skeleton: 'from django.db import models\nfrom django.db.models import F\nfrom django.db.models.fields.json import ____\n\n\nclass Product(models.Model):\n    name = models.CharField(max_length=200)\n    price = models.DecimalField(max_digits=10, decimal_places=2)\n    metadata = models.____(default=dict, blank=True)\n\n    def __str__(self):\n        return self.name\n\n\n# --- Example queries ---\n\nred_products = Product.____.____(____="red")\n\nwide_products = Product.____.____(____=10)\n\nproducts_with_color = Product.____.____(metadata__has_key="color")\n\norganic = Product.____.____(\n    metadata__contains={"organic": True, "origin": "USA"}\n)\n\nproducts_by_brand = Product.____.____(\n    brand=____("brand", "metadata")\n).____(brand__isnull=False).____("brand")',
      },
...
class ArticleSearchView(APIView):
    def get(self, request):
      ...
      tieredHints: {
        apiSignature: 'SearchVector(*fields, weight=None); SearchQuery(value); SearchRank(vector, query)',
        skeleton: 'from django.db import models\nfrom django.contrib.postgres.search import (\n    ____,\n    ____,\n    ____,\n    ____,\n)\nfrom rest_framework.views import APIView\nfrom rest_framework.response import Response\n\n\nclass Article(models.Model):\n    title = models.____(max_length=300)\n    body = models.____()\n    author = models.____("auth.User", on_delete=models.CASCADE)\n    published_at = models.____(auto_now_add=True)\n\n    def ____(self):\n        return ____.title\n\n\nclass ArticleSearchView(APIView):\n    def ____(self, request):\n        query_text = request.query_params.____("q", "")\n        if not ____:\n            return ____({"results": []})\n\n        ____ = ____("title", weight="A") + ____("body", weight="B")\n        ____ = ____(query_text)\n\n        results = (\n            Article.____\n            .____(\n                search=____,\n                rank=____(____, ____),\n                headline=____("body", ____, max_words=35, min_words=15),\n            )\n            .____(search=____)\n            .____(rank__gte=0.1)\n            .____("-rank")\n        )\n\n        data = [\n            {\n                "id": ____.id,\n                "title": ____.title,\n                "headline": ____.headline,\n                "rank": float(____.rank),\n            }\n            for ____ in results[:20]\n        ]\n\n        return ____({"results": ____})',
      },
...
`,
      ...
      tieredHints: {
        apiSignature: 'class Meta: indexes = [models.Index(fields=[], name="")]',
        skeleton: 'from django.db import models\n\nclass Order(models.Model):\n    order_id = models.____(primary_key=True)\n    customer = models.____("Customer", on_delete=models.CASCADE, ____=True)\n    order_status = models.____(max_length=____)\n    created_at = models.____(auto_now_add=True)\n    total_amount = models.____(max_digits=____, decimal_places=____)\n\n    class ____:\n        ____ = [\n            models.____(fields=["____", "____"], name="idx_status_created"),\n        ]',
      },
    ...
    },
  {
      ...
      tieredHints: {
        apiSignature: 'EXPLAIN ANALYZE <sql_query>; CREATE INDEX <name> ON <table> (<columns>);',
        skeleton: '____ ____ ____ * ____ ____ ____ ____ \'active\' ____ ____ > \'2024-01-01\';\n\n____ ____ idx_orders_status_date ____ ____(____, ____);',
      },
...
def search(term):
    ...
    return (
      ...
      tieredHints: {
        apiSignature: 'SearchQuery(value, search_type=\'plain\', config=None)',
        skeleton: 'from django.contrib.postgres.search import ____, ____, ____\nfrom .models import Article\n\n\ndef search(term):\n    vector = ____("title", weight="A") + ____("body", weight="B")\n    query = ____(term, search_type="websearch")\n    return (\n        Article.____\n        .____(rank=____(vector, query))\n        .____(rank__gt=0)\n        .____("-rank")\n    )',
      },
...
def admin_big_order_events():
    return (
      ...
      tieredHints: {
        apiSignature: 'QuerySet.filter(**kwargs).order_by(*fields)',
        skeleton: 'from .models import Event\n\n\ndef admin_big_order_events():\n    return (\n        Event.____\n        .____(____="order_created")\n        .____(____="admin")\n        .____(____=100)\n        .____("-created_at")\n    )',
      },
